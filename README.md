...
- **Chat de texto vía TCP** (confiable, orientado a conexión).
- **Streaming de video por frames vía UDP** (baja latencia, tolera pérdida).

Fork base del profesor: `Chat-TCP-UDP-base` (`COKII5/Chat-TCP-UDP-base`).
...
2. Levantar primero la instancia "servidor" (chat + video), luego la
   instancia "cliente" (ej. build + Editor, o dos builds en la misma red).
3. Chat: conectar con la IP del servidor y puerto `5555`.
4. Video: el receptor se conecta al puerto `5000`; espera el handshake del
   emisor antes de empezar a recibir frames.

## Estado / pendientes
- Fases 0–5 completas (protocolo, fork, envío/recepción de video, chat TCP
  con UI, limpieza de sockets/concurrencia). Ver historial de commits.
- **Fase 6 (UI integrada):** falta unir chat + video en una sola escena por
  rol (cliente / servidor) y asignar en el Inspector el campo `chatDisplay`
  de `UI_TCPClient`/`TCPServerUI` — trabajo manual en el Editor de Unity,
  no automatizable desde código.
- **Fase 7:** plan de pruebas manual en
  [`docs/Fase7-plan-pruebas.md`](docs/Fase7-plan-pruebas.md), pendiente de
  ejecutar tras armar la Fase 6.
